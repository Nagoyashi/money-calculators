...
| `v0.15.2` | 2026-07-15 | Tracker debug + polish sweep: grid data-loss fix (background refetch), recurring projections in the grid + "Apply", category rename/drag-reorder/20-cap/collapsed archive, comma decimals (`NumInput` → text input), Overview avg-median on both cards + income pie + forecast-month explanation, Transactions pagination (30/page) + richer filters + form-above-list, Budgets teaser tab. Suites 231 frontend / +3 backend. → [v0.15.2](docs/releases/v0.15.2.md) |
| `v0.15.3` | 2026-07-15 | Marketing redesign: the landing page moves to a light fintech design (white/gray-50/blue-600, amber brand accent) with rewritten copy, an interactive `HeroAppPreview` app miniature, light calculator showcase + tracker cards, a slate-900 CTA band, and a light restyle of the in-app sidebar (`AppSidebar`/`UserFooter`/`SavedCalculationsSidebar`). Publish defaults now mirror the completed rollout — all 12 calculators + both trackers default published (fallback + fresh-DB seed; live DBs untouched). Suite 237 frontend. → [v0.15.3](docs/releases/v0.15.3.md) |
| `v0.15.4` | 2026-07-16 | Guide, Comparison & ETFs & Stocks draft pages (from the design handoff) live in the bundle behind a `SuperadminPreview` route gate — public visitors keep the coming-soon pages; superadmins see the drafts with an amber preview banner. Shared chrome: nav active-link state, slim footer variant, `MarketingPageHero`. Suite 257 frontend. → [v0.15.4](docs/releases/v0.15.4.md) |
| `v0.15.5` | 2026-07-26 | Tracker history control: Net Worth snapshots become editable/deletable (`PUT`/`DELETE /snapshots/<id>`, net worth recomputed server-side, manage-list under the trend chart — #318) and the I&E Overview gains a **Recurring** panel (all active rules + next occurrence, one-click stop, edit handoff — #317). → [v0.15.5](docs/releases/v0.15.5.md) |
